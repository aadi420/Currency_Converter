...
$(document).ready(function () {
  ...
  function getExchangeRate() {
    ...
    let url = `https://v6.exchangerate-api.com/v6/64aa816c0f61933f4018903a/latest/${fromCurrency.find(":selected").val()}`;
      ...
      .then((result) => {
        ...
        $("form .exchange-rate").html(
          `${amountVal} ${fromCurrency
            .find(":selected")
            .val()} = ${totalExRate} ${toCurrency.find(":selected").val()}  <span class = ${colorClass}> (${RandomNum})% </span>   `
        );
      })
      .catch(() => {
